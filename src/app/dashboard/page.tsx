...
export default async function DashboardPage() {
  ...
  const themeCookie = cookieStore.get('dashboard-theme')?.value
  ...
  const [{ data: courses }, { data: dashboardAnnouncement }] = await Promise.all([
    query,
    supabase
      .from('announcements')
      .select('text')
  ...
  return (
    <div className="dashboard-root" data-theme={theme} style={{ background: 'var(--color-canvas)', minHeight: '100vh' }}>
      {/* Dashboard Top Bar */}
      <nav style={{
      ...
      {dashboardAnnouncement?.text && (
        <DashboardAnnouncementBar text={dashboardAnnouncement.text} />
      )}
      ...
      <header style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
            ...
            <DashboardThemeToggle initialTheme={theme} />
          </div>
        </div>
      </header>
